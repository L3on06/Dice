let player1: string = "";
let player2: string = "";
let namesEntered: boolean = false;

const buttonStart: HTMLElement | null = document.querySelector("#buttonstart");

const buttonEnd: HTMLElement | null = document.querySelector("#buttonend");

function validateNameType(playerName: string): boolean {
  const regex: RegExp = /^[a-zA-Z]+$/;
  return regex.test(playerName);
}

function validateNameZeroSymbol(playerName: string): boolean {
...
if (buttonStart && buttonEnd) {
  ...
  buttonEnd.addEventListener('click', () => {
    if (buttonEnd && buttonStart) {
      ...
      const image1 = document.querySelector(".img1") as HTMLImageElement;
      if (image1) {
      ...
      const image2 = document.querySelector(".img2") as HTMLImageElement;
      if (image2) {
